...
@Component({
    selector: 'app-low-tunnel-post-verification',
    templateUrl: './low-tunnel-post-verification.page.html',
    styleUrls: ['./low-tunnel-post-verification.page.scss'],
})
export class LowTunnelPostVerificationPage implements OnInit {
    ...
    takePhotoDirect() {
        this.camera.getPicture(this.httpClient.options).then(
            (imageData) => {
                ...
                if (this.httpClient.isOffline) {
                    this.dbService.storage.executeSql('SELECT * FROM postVerificationOfflineOfLowTunnel WHERE ApplicationId = ?',
                        [this.data.ApplicationId]).then((res) => {
                            ...
                            this.dbService.storage.executeSql(`UPDATE postVerificationOfflineOfLowTunnel SET applicationData = ? WHERE ApplicationId = ${this.data.ApplicationId}`, data)
                                .then(() => {
    ...
    takePhotoNoCase(param, columnName) {
        ...
        this.camera.getPicture(this.httpClient.options).then(
            (imageData) => {
                ...
                if (this.httpClient.isOffline) {
                    this.dbService.storage.executeSql("SELECT * FROM postVerificationOfflineOfLowTunnel WHERE ApplicationId = ?", [this.data.ApplicationId]).then((res) => {
                        console.log("second");
                        var temp = JSON.parse(res.rows.item(0).applicationData);
                        console.log("third");
                        if (param == 1) {
                        ...
                        let data = temp;
                        this.dbService.storage.executeSql(`UPDATE postVerificationOfflineOfLowTunnel
