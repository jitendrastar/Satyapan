...
export class AaoVerificationPage {
  ...
  constructor(public actionSheetController: ActionSheetController, 
    public route: ActivatedRoute, public navCtrl: NavController, public modalCtrl: ModalController, public router: Router, public alertCtrl: AlertController, public callNumber: CallNumber, public storage: Storage, public httpClient: CommonService) {
       

    this.storage.get('subsidyData').then(valueSubsidy => {
      if (valueSubsidy) {
          this.subsidylist = valueSubsidy;
          console.log(this.subsidylist);
 ...
 // console.log(this.header);
  ...
  ionViewWillEnter() {
    ...
    this.subsidyids=this.subsidyids;
 
     ///  alert(1);
       if(this.subsidyid){
       // this.getPreVerificationAssignListMobile(this.roleId, this.userid,this.subsidyid);
        if(this.header==2){
          // alert(2);
           this.getPreVerificationAssignListMobileAAO(this.roleId, this.userid,this.subsidyid);
         }else{
           //alert(1);
          this.getPreVerificationAssignListMobile(this.roleId, this.userid,this.subsidyid);
         }
      ...
      /* if(this.subsidyids){
        this.getPreVerificationAssignListMobile(this.roleId, this.userid,this.subsidyids);
       }else{
        this.getPreVerificationAssignListMobile(this.roleId, this.userid,this.subsidyid);
       }*/
  ...
  onchangeSubsidy(subsidyid){
    //alert(subsidyid);
    //this.roleId = this.httpClient.userData.roleid;
    //this.userid = this.httpClient.userData.userid;
    if(this.header==2){
     // alert(2);
      this.getPreVerificationAssignListMobileAAO(this.roleId, this.userid,subsidyid);
    }else{
      //alert(1);
     this.getPreVerificationAssignListMobile(this.roleId, this.userid,subsidyid);
    }
    
  }
  getPreVerificationAssignListMobileAAO(roleId, userid,subsidyid) {
    var self = this;
    ...
    var sendRequestData = {
      obj: {
        ...
        srvmethodnm: "GetApplicationListToAssignAS",
        srvparam: "{'roleid':'" + roleId + "','userid':'" + userid + "','schemeid':'" + subsidyid + "','SubsidySchemeId':'" + subsidyid + "'}",
      },
    };
      ...
      (error) => {
        var errorRequestData = {
          obj: {
            ...
            srvmethodnm: "GetApplicationListToAssignAS",
            srvparam: "{'roleid':'" + roleId + "','schemeid':'" + 1 + "','userid':'" + userid + "'}",
          },
        };
  ...
  getPreVerificationAssignListMobile(roleId, userid,subsidyid) {
    this.subsidyids = subsidyid; 
    var self = this;
    self.httpClient.showLoadingImage();
    self.getPreVerificationAssignListMobileData=[];
    var sendRequestData = {
      obj: {
        ...
        srvparam: "{'roleid':'" + roleId + "','userid':'" + userid + "','SubsidySchemeId':'" + subsidyid + "'}",
      },
    };
    var tempArrayOfPendingList: any = [];
    this.httpClient.post(sendRequestData).subscribe(
    ...
    if(this.header==2){
      // alert(2);
       ...
       this.router.navigate([
        "assign-submission-aao",
        { dataForAAO: JSON.stringify(data) },
      ]);
     }else{
     this.router.navigate([
      "assign-submission",
