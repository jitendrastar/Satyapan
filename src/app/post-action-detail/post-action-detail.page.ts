...
export class PostActionDetailPage {
   ...
   longitude:any;
    ...
    imgAS: any;
    isRWSLIP: any;
    btnYesForm:boolean = false;
    btnFinalSubmit:boolean = false;
    btnNoForm:boolean = false;

    constructor(public photoViewer: PhotoViewer, public locationAccuracy: LocationAccuracy, public platform: Platform,  public transferObject: FileTransferObject, public file: File, public dbService: DatabaseServiceService, public fileOpener: FileOpener, public downloader: Downloader, public sanitizer: DomSanitizer, public modalController: ModalController, public zone: NgZone, public navCtrl: NavController, public actionSheetController: ActionSheetController, public storage: Storage, public route: ActivatedRoute, public loadCtrl: LoadingController, public geolocation: Geolocation, public document: DocumentViewer, public camera: Camera, public httpClient: CommonService, public alertController: AlertController) {
        
        this.getPreVerificationData = JSON.parse(this.route.snapshot.paramMap.get("obj"));
        console.log("getPreVerificationData = "+JSON.stringify(this.getPreVerificationData));
        this.imgJamabandi = this.getPreVerificationData.ImgJamabandi.split(",");
       // alert(this.getPreVerificationData.ImgAS);
       // this.imgAS = this.getPreVerificationData.ImgAS;
        this.isRWSLIP = this.getPreVerificationData.ISRWSLIP == 1 ? 'Yes' : 'No';
    ...
    select_loc_withIMG_farmer(flags,columnNames){
        this.get_loc();
       // if (this.lat == null || this.lat == ""|| this.lat == undefined) {
   if (this.longitude == null ||this.longitude == null ||this.longitude  == null) {
                      ...
                      this.takePhoto(flags, columnNames);
                  }
    }
    ...
    submitYesForm() {
        this.btnYesForm = true;
        console.log("isRWSLIP", this.isRWSLIP);
        ...
        if (this.latitude == null || this.latitude == undefined || this.latitude == "") {
            ...
            this.btnYesForm = false;
        } else if (this.longitude == null || this.longitude == undefined || this.longitude == "") {
            if (this.httpClient.currentLanguage == "hindi") {
                this.httpClient.showToast(
                    "कृपया इस एप्लिकेशन के लिए लोकेशन सक्षम करें फिर सबमिट करें"
                );
            ...
            var sendRequestData = {
                obj: {
                    ...
                    srvparam: JSON.stringify({
                        ...
                        isRWSLIP: self.isRWSLIP,
                    }),
                },
            };
            console.log("ISRWSLIP"+ self.isRWSLIP);
            console.log("sendRequestData", sendRequestData);
            if (this.httpClient.isOffline == true) {
    ...
    takePhotoNakshaTrace() {
        this.camera.getPicture(this.httpClient.options).then((imageData) => {
            ...
            if (!this.httpClient.isOffline) {
                this.httpClient.showLoading();
                FileTransfer.create().upload(imageData, this.httpClient.imageUploadUrlForJamaBandi, options).then(
                    (data) => {
                        this.httpClient.dismissLoading();
                        // success
