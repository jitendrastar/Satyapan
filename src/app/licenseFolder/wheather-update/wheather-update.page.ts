...
@Component({
  selector: 'app-wheather-update',
  templateUrl: './wheather-update.page.html',
  styleUrls: ['./wheather-update.page.scss'],
})
export class WheatherUpdatePage implements OnInit {
  ...
  //  "
  constructor(public photoViewer: PhotoViewer,  public file: File, public camera: Camera, public geolocation: Geolocation,
  ...
  getLoc() {
    ...
    this.geolocation.getCurrentPosition().then((resp) => {
      ...
      this.httpClient.dismissLoadingImage();
      ...
      function (temp) {
        ...
        if (res.status == 0) {
         ...
         // alert( self.kskLocation);
          ...
          self.outsidePhoto = self.firstData.image;
   ...
   // alert();
    ...
    var sendRequestData = {
      obj: {
        ...
        srvmethodnm: "GetWeatherStationListbyPS",
...
back(){
  ...
  submit(submitType) {
   ...
   // console.log("assetsArray", assetsArray);
    ...
    else if (!this.wheatherstation  && this.panchayatSamiti) {
      this.httpClient.showToast((this.httpClient.currentLanguage == 'hindi') ? 'कृपया मौसम स्टेशन चुनें का चयन करें' : 'Please select the Wheather Station');
    }
    else if (!this.name) {
      this.httpClient.showToast((this.httpClient.currentLanguage == 'hindi') ? 'कृपया मौसम स्टेशन के मालिक का नाम दर्ज करें' : 'Please enter the OwnerName of Wheather Station');
    }
    else if (!this.mobile) {
      this.httpClient.showToast((this.httpClient.currentLanguage == 'hindi') ? 'कृपया मौसम स्टेशन का मोबाइल नंबर दर्ज करें' : 'Please enter the mobile number of Wheather Station');
    }
    else if (this.mobile.toString().length > 10) {
      // this.mobile = "";
       this.httpClient.showToast((this.httpClient.currentLanguage == 'hindi') ? 'कृपया मौसम स्टेशन का वैध मोबाइल नंबर दर्ज करें' : 'Please enter the valid mobile number of Weather Station');
     }
     else if (this.mobile.toString().length < 10) {
      // this.mobile = "";
       this.httpClient.showToast((this.httpClient.currentLanguage == 'hindi') ? 'कृपया मौसम स्टेशन का वैध मोबाइल नंबर दर्ज करें' : 'Please enter the valid mobile number of Weather Station');
     }
    /*else if (!this.wheatherstation && this.isKnownFor) {
      ...
      this.httpClient.showToast((this.httpClient.currentLanguage == 'hindi') ? 'कृपया वर्किंग स्टेशन विकल्प  का चयन करें' : 'Please select the  Working Station option');
    }*/
  /*  else if (!this.electricConnection) {
   ...
   /* else if (!this.aaoPosted) {
      ...
      this.httpClient.showToast((this.httpClient.currentLanguage == 'hindi') ? 'कृपया  फोटो अपलोड करें' : 'Please upload  Photo');
    }
    else {
     // alert(this.outsidePhoto);
     // alert();
     // return false;
      ...
      var self = this;
      if(self.isRempAtEntryGate){
        self.wheatherstationval=1;
      }else{
        self.wheatherstationval=0;
      }
      this.httpClient.showLoading();
      var sendRequestData = {
        "obj": {
          ...
          "psw": "rajkisan@123",
          "srvnm": "PreVerification",
          "srvmethodnm": "SaveWeatherStationInfo",
          "srvparam": JSON.stringify({
            'userid': self.httpClient.userData.userid,
            'StationId': self.wheatherstation,
            'IsWorkingStation': self.wheatherstationval,
            'image': self.outsidePhoto,
            'latitude': self.lat,
            'longitude': self.long,
  ...
  setenableyes(event){
    this.enableyes=true;
    var strname = $(".for_select2"+event).attr('lang');
    //alert(strname);
    var strid= $(".for_select2input"+event).attr('lang');
