...
import { AddGodownPage } from '../model/add-godown/add-godown.page';
...
export class ApplicationVerificationPage implements OnInit {
    ...
    constructor(
        ...
        public httpClient: CommonService, public downloader: Downloader,
    ...
    submit() {
        ...
        if (this.checkList[0].IsPVDone == "False") {
            // alert("in");
            if (this.createdChecklist.length <= 0) {
                if (this.httpClient.currentLanguage == 'hindi') {
    ...
    openPDF(item) {
        ...
        if (extension === "jpg" || extension === "png" || extension === "jpeg") {
            ...
            /* var URL = encodeURI(item);
             ...
             FileTransfer.create().download(URL, filePath).then((entry) => {
        ...
        var sendRequestData = {
            'obj': {
                ...
                srvparam: JSON.stringify({
                    ...
                    userid: self.httpClient.userData.userid,
                    ApplicationId: self.checkList[0].ApplicationId,
                    AppType: self.upcomingData.ApplicationType,
                    Remarks: 1,
    ...
    async addGodown() {
        ...
        modal.onDidDismiss().then((dataReturned) => {
            console.log("Member List to besubmitted = " + JSON.stringify(dataReturned));
            if (dataReturned.role == "backdrop") {
            } else {
                // this.router.navigate(['/successfull-verified']);
            }
        });
