...
@Component({
  selector: 'app-mbsy-cal-allotment',
  templateUrl: './mbsy-cal-allotment.page.html',
  styleUrls: ['./mbsy-cal-allotment.page.scss'],
})
export class MbsyCalAllotmentPage implements OnInit {
  ...
  constructor(public httpClient: CommonService, public route: ActivatedRoute, public alertCtrl: AlertController, public navCtrl: NavController) {
    ...
    if (item.isProducerFarmer != isPrevValue) {
      if (item.isProducerFarmer) {
        const prompt = await this.alertCtrl.create({
          header: this.httpClient.currentLanguage == "hindi" ? "क्या आपको यकीन है, यह किसान उत्पादक किसान है?" : "Are you sure, This farmer is producer farmer?",
          message: this.httpClient.currentLanguage == "hindi" ? "Area to be allocated for current Season" : "इस सीजन के लिए आवंटित किया जाने वाला क्षेत्र",
          backdropDismiss: false,
          cssClass: 'producer-farmer-alert',
          inputs: [{
            name: 'allotedCurrentArea',
            type: 'tel',
            placeholder: 'Area to be allocated for current Season',
            attributes: {
              required: true
            }
          }],
          buttons: [{
            ...
            handler: (data) => {
              ...
              if (!data.allotedCurrentArea) {
                this.httpClient.showToastMsg('Please Enter Area to be allocated for current Season.');
                return false;
              } else {
  ...
  finalSaveGroupDataPrompt() {
    ...
    this.httpClient.post(sendRequestData).subscribe(
      function (temp) {
        ...
        self.httpClient.dismissLoading();
        if (res.status == 0) {
          self.httpClient.showToast(res.data[0].MSG);
          self.navCtrl.pop();
        } else {
          self.httpClient.showToast(res.message);
        }
