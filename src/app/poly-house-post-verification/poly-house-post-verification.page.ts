...
@Component({
    selector: 'app-poly-house-post-verification',
    templateUrl: './poly-house-post-verification.page.html',
    styleUrls: ['./poly-house-post-verification.page.scss'],
})
export class PolyHousePostVerificationPage implements OnInit {
    ...
    isslected_land_for_dealers: any;
    ...
    takePhotoDirect() {
        this.camera.getPicture(this.httpClient.options).then(
            (imageData) => {
                ...
                if (this.httpClient.isOffline) {
                    this.dbService.storage.executeSql('SELECT * FROM postVerificationOfflineOfPolyHouse WHERE ApplicationId = ?',
                        [this.data.ApplicationId]).then((res) => {
                            ...
                            this.dbService.storage.executeSql(`UPDATE postVerificationOfflineOfPolyHouse SET applicationData = ? WHERE ApplicationId = ${this.data.ApplicationId}`, data)
                                .then(() => {
    ...
    getAddSprinklerPostVerification() {
        //alert( this.isslected_land_for_dealer);
        // return false;
    ...
    takePhotoNoCase(param, columnName) {
        ...
        this.camera.getPicture(this.httpClient.options).then(
            (imageData) => {
                ...
                if (this.httpClient.isOffline) {
                    this.dbService.storage.executeSql("SELECT * FROM postVerificationOfflineOfPolyHouse WHERE ApplicationId = ?", [this.data.ApplicationId]).then((res) => {
                        console.log("second");
                        var temp = JSON.parse(res.rows.item(0).applicationData);
                        console.log("third");
                        if (param == 1) {
                        ...
                        let data = temp;
                        this.dbService.storage.executeSql(`UPDATE postVerificationOfflineOfPolyHouse
