...
export class SubsidyDetailPage {
    ...
    goToDetails(dataTosend, mobile) {
        ...
        } else if (data.SubsidySchemeId == "12") {
            if (data.sendRequestDataFinal) {
                this.httpClient.showToast(
                    "This verification is already done. Please SYNC this verification"
                );
                ...
                this.router.navigate([
                    "barbed-wire-fencing-pre-verification",
                    { obj: JSON.stringify(data) },
                ]);
            }
        } else if (data.SubsidySchemeId == "62") {
            if (data.sendRequestDataFinal) {
                this.httpClient.showToast(
                    "This verification is already done. Please SYNC this verification"
                );
            ...
            function (res: any) {
                if (res[0].status == 0) {
                    ...
                    for (let i = 0; i < self.preVerificationListData.length; i++) {
                        ...
                        if (self.preVerificationListData[i].IsAllPVDone == 1) {
                            self.alldone = true;
                        }
                    }
    ...
    submitCheckList(data) {
        if (this.subsidyId == "1") {
            ...
            if (data.mainDataSubmit != undefined) {
                ...
                FileTransfer.create().upload(data.imageOFimgFarmerWithOfficerNoCase, this.httpClient.imageUploadUrl, data.optionsOFimgFarmerWithOfficerNoCase).then((imgfarmerNoCase) => {
                    ...
                    FileTransfer.create().upload(data.imageOFfarmerNonConsentLetter, this.httpClient.imageUploadUrl, data.optionsOFfarmerNonConsentLetter).then((consentLetter) => {
    ...
    uploadNajariyaNaksha(preVerificationID, data) {
        if (this.subsidyId == "1") {
            ...
            FileTransfer.create().upload(data.imageOFnajariyaNaksha, this.httpClient.imageUploadUrl, options).then((najariyaNaksha) => {
            ...
            options.params.id = preVerificationID;
            console.log("options najariya naksha", options);
            FileTransfer.create().upload(data.imageOFImgSignedMap, this.httpClient.imageUploadUrl, options).then((najariyaNaksha) => {
                var temp = JSON.parse(najariyaNaksha.response);
                console.log("najariya naksha uploaded", temp[0].data);
                this.uploadFarmerImage(preVerificationID, data);
            }),
    ...
    uploadFarmerImage(preVerificationID, data) {
        ...
        if (this.subsidyId == "1") {
            ...
            FileTransfer.create().upload(data.imageOFimgFarmerAndOfficerWithFarmPond, this.httpClient.imageUploadUrl, options).then((Officer) => {
            ...
            options.params.id = preVerificationID;
            console.log("options  farmer Officer", options);
            FileTransfer.create().upload(data.imageOFImgfarmerWithOfficer, this.httpClient.imageUploadUrl, options).then((Officer) => {
                var temp = JSON.parse(Officer.response);
                console.log(" farmer Officer", temp[0].data);
                if (self.subsidyId == "1") {
                    self.submitFinalDataOfFarmPond(data);
            ...
            options.params.id = preVerificationID;
            console.log("options  farmer Officer", options);
            FileTransfer.create().upload(data.imgFarmerAndOfficerWithFarmPond, this.httpClient.imageUploadUrl, options).then((Officer) => {
                var temp = JSON.parse(Officer.response);
                console.log(" farmer Officer", temp[0].data);
                self.submitFinalDataOfFarmPond(data);
    ...
    uploadSourceOfIrrigration(preVerificationID, data) {
        if (this.subsidyId == "5") {
            ...
            options.params.id = preVerificationID;
            console.log("options  farmer Officer", options);
            FileTransfer.create().upload(data.imageOFImgSourceOfIrrigation, this.httpClient.imageUploadUrl, options).then((Officer) => {
                var temp = JSON.parse(Officer.response);
                console.log(" farmer Officer", temp[0].data);
    ...
    */
    ...
    submitFinal() {
        //alert($("#finalremark").val());//this.finalremark);
        // console.log(this.finalremark);
        // console.log(self.finalremark);
        var finaltxt = $("#finalremark").val();
        if (!finaltxt) {
            if (this.httpClient.currentLanguage == "english") {
                this.httpClient.showToast("Please Enter Remark");
            } else {
                this.httpClient.showToast("कृपया रिमार्क दर्ज करें ");
            }
        } else {
            var self = this;
            var sendRequestData = {
                obj: {
                    ...
                    srvparam: JSON.stringify({
                        ...
                        //  this.roleId, this.userid, this.subsidyId,this.cast
