...
@Component({
  selector: "app-orchard-post-verification",
  templateUrl: "./orchard-post-verification.page.html",
  styleUrls: ["./orchard-post-verification.page.scss"],
})
export class OrchardPostVerificationPage {
  remarks: any = "";
  ...
  orchardList: any[] = [];
  selectedOrchardType: any;
  selectedCropType: any;
  orchardPlantDistanceList: any[] = [];
  selectedPlantDistId: any;
  orchardCostObj: any;
  actualTotalPlants: any;
  alreadySubsidyYear: any;
  alreadySubsidyArea: any;
  alreadySubsidyPlantName: any;
  ...
  getOrchardList() {
    ...
    credObj.obj.srvnm = "PostVerification";
    credObj.obj.srvmethodnm = "GetOrchardList";
    credObj.obj.srvparam = JSON.stringify({});
    self.httpClient.post(credObj).subscribe((res) => {
      if (res[0].status == 0) {
        this.httpClient.dismissLoading();
        self.orchardList = res[0].data;
        setTimeout(() => {
          self.selectedOrchardType = this.data.OrchardId + '';
        }, 200);
      } else {
  ...
  getOrchardPlantDistanceList(selectedCrop) {
    this.httpClient.showLoading();
    var self = this;
    let credObj: any = new Object();
    credObj.obj = new Object();
    ...
    credObj.obj.psw = "rajkisan@123";
    credObj.obj.srvnm = "PostVerification";
    credObj.obj.srvmethodnm = "GetOrchardPlantDistanceList";
    credObj.obj.srvparam = JSON.stringify({ CropType: selectedCrop, OrchardID: self.selectedOrchardType });
    self.httpClient.post(credObj).subscribe((res) => {
      if (res[0].status == 0) {
        this.httpClient.dismissLoading();
        self.orchardPlantDistanceList = res[0].data;
      } else {
        this.httpClient.showToast(res[0].message);
        this.httpClient.dismissLoading();
      }
  ...
  getOrchardCostByid() {
    ...
    credObj.obj.psw = "rajkisan@123";
    credObj.obj.srvnm = "PostVerification";
    credObj.obj.srvmethodnm = "GetOrchardCostByid";
    credObj.obj.srvparam = JSON.stringify({ OrchardID: self.selectedOrchardType, PlantDistanceID: self.selectedPlantDistId });
    self.httpClient.post(credObj).subscribe((res) => {
      if (res[0].status == 0) {
        this.httpClient.dismissLoading();
        self.orchardCostObj = res[0].data[0];
        console.log(self.data.LandAreaForSubsidy);
        console.log(Number(self.data.LandAreaForSubsidy) % Number(self.orchardCostObj.TotalPlant));
        
        
        self.orchardCostObj.TotalPlantForTotalLand = (Number(self.orchardCostObj.TotalPlant) / 100 * Number(self.data.LandAreaForSubsidy) * 100).toFixed(2);
        self.orchardCostObj.EstCostWithDripForTotalLand = (Number(self.orchardCostObj.EstCostWithDrip) / 100 * Number(self.data.LandAreaForSubsidy) * 100).toFixed(2);
        self.orchardCostObj.EstCostWithoutDripForTotalLand = (Number(self.orchardCostObj.EstCostWithoutDrip) / 100 * Number(self.data.LandAreaForSubsidy) * 100).toFixed(2);
      } else {
        this.httpClient.showToast(res[0].message);
        this.httpClient.dismissLoading();
      }
  ...
  getAddSprinklerPostVerification() {
    ...
    } else if (!this.selectedOrchardType) {
      msg =
        this.httpClient.currentLanguage == "hindi"
          ? "कृपया बाग के प्रकार का चयन करें"
          : "Please select the Orchard type.";
      this.httpClient.showToast(msg);
      this.btnAddSprinkler = false;
    } else if (!this.selectedCropType) {
      msg =
        this.httpClient.currentLanguage == "hindi"
          ? "कृपया फसल के प्रकार का चयन करें"
          : "Please select the Crop type.";
      this.httpClient.showToast(msg);
      this.btnAddSprinkler = false;
    } else if (!this.selectedPlantDistId) {
      msg =
        this.httpClient.currentLanguage == "hindi"
          ? "कृपया पौधों के बीच की दूरी का चयन करें"
          : "Please select the Distance between Plants.";
      this.httpClient.showToast(msg);
      this.btnAddSprinkler = false;
    } else if (!this.actualTotalPlants) {
      msg =
        this.httpClient.currentLanguage == "hindi"
          ? "कृपया पौधों की वास्तविक संख्या दर्ज करें"
          : "Please enter the actual count of Plants.";
      this.httpClient.showToast(msg);
      this.btnAddSprinkler = false;
    } else if (this.getVerificationCheckData.length != self.ChecklistDetailList.length) {
      if (this.httpClient.currentLanguage == "hindi") {
        ...
        var sendRequestData = {
          obj: {
            ...
            srvparam: JSON.stringify({
              ...
              optionalArray: this.optionalArray,
              PrevAvailedSubsidyArray: this.prevAvailedSubsidyArray,
              HortiOCDTotalPlant : this.orchardCostObj.TotalPlant,
              HortiOCDActualTotalPlant : this.actualTotalPlants,
              HortiOCDEstCostWithDrip : this.orchardCostObj.EstCostWithDrip,
              HortiOCDEstCostWithoutDrip : this.orchardCostObj.EstCostWithoutDrip,
              HortiOCDCostID : this.orchardCostObj.OrchardCostID,
              HortiOCDOrchardID : this.selectedOrchardType,
              HortiAlreadySubsidyYear: this.alreadySubsidyYear,
              HortiAlreadySubsidyAmt: this.alreadySubsidyAmount,
              HortiAlreadySubsidyLandArea: this.alreadySubsidyArea,
              HortiAlreadyOCDName: this.alreadySubsidyPlantName,
              HortiOCDSubsidyYearCount: 0
            }),
          },
        };
