...
@Component({
  selector: "app-solar-post-verification",
  templateUrl: "./solar-post-verification.page.html",
  styleUrls: ["./solar-post-verification.page.scss"],
})
export class SolarPostVerificationPage {
  remarks: any = "";
  ...
  solarPumpPlant: any;
  solarPumpType: any;
  solarSubmersibalPump: any;
  solarControllerType: any;
  solarPumpCapacity: any;
  solarPumpWaterSource: any;
  isAlreadySubsidy: any;
  solarMicroIrrigation: any;
  solarHorticultureActivity: any;
  solarIrrigatedLandArea: any;
  solarSowedCropDetails: any;
  solarExperimentalTest: any;
  solarAnyOtherPoints: any;
  arrSolarMicroIrrigation: any[] = [];
  arrSolarHorticultureActivity: any[] = [];
  ...
  constructor(
    ...
    if (!this.httpClient.isOffline) {
      ...
      if (parseInt(this.data.PostVerificationId) > 0) {
        ...
        if (this.data.ImgFarmerWithOfficer3 != "") {
          this.ImgFarmerWithOfficer3 = this.data.ImgFarmerWithOfficer3;
        }
        if (this.data.ImgSolar10_2 != "") {
          this.ImgSolar10_2 = this.data.ImgSolar10_2;
        }
        if (this.data.ImgSolar10_2 != "") {
          this.ImgSolar10_2 = this.data.ImgSolar10_2;
        }
  ...
  viewSolarManufactureBill() {
    window.open(this.data.solarManufactureBill);
  }
  ...
  ionViewWillEnter() {
    ...
    this.getSolarPVIrrigationSystem();
    this.getLoc();
  }

  getSolarPVIrrigationSystem() {
    ...
    var sendRequestData = {
      obj: {
        ...
        srvparam: JSON.stringify({
          AgriLovValuesCode: 'SolarPVIrrigationSystem'
        }),
      },
    };
    self.httpClient.post(sendRequestData).subscribe((res) => {
      this.httpClient.dismissLoading();
      if (res[0].status == 0) {
        self.arrSolarMicroIrrigation = res[0].data;
        for (var i = 0; i < self.arrSolarMicroIrrigation.length; i++) {
          self.arrSolarMicroIrrigation[i].isChecked = false;
        }
        self.getSolarPVHorticultureActivities();
      } else {
  ...
  getSolarPVHorticultureActivities() {
    this.httpClient.showLoading();
    var self = this;
    var sendRequestData = {
      obj: {
        ...
        srvmethodnm: "agrilovvalues",
        srvparam: JSON.stringify({
          AgriLovValuesCode: 'SolarPVHorticultureActivities'
        }),
      },
    };
    self.httpClient.post(sendRequestData).subscribe((res) => {
      this.httpClient.dismissLoading();
      if (res[0].status == 0) {
        self.arrSolarHorticultureActivity = res[0].data;
        for (var i = 0; i < self.arrSolarHorticultureActivity.length; i++) {
          self.arrSolarHorticultureActivity[i].isChecked = false;
        }
      } else {
        this.httpClient.showToast(res[0].message);
        this.httpClient.dismissLoading();
  ...
  getAddSprinklerPostVerification() {
    console.log('ChecklistDetailList : ', this.ChecklistDetailList);
    ...
    if (this.latitude == null || this.longitude == null) {
      ...
      this.btnAddSprinkler = false;
      // } else if (!this.solarPumpPlant || !this.solarPumpType || !this.solarSubmersibalPump || !this.solarControllerType || !this.solarPumpCapacity || !this.solarPumpWaterSource || !this.isAlreadySubsidy || !this.solarMicroIrrigation || !this.solarHorticultureActivity || !this.solarIrrigatedLandArea || !this.solarSowedCropDetails || !this.solarExperimentalTest || !this.solarAnyOtherPoints) {
      //   if (this.httpClient.currentLanguage == "hindi") {
      //     this.httpClient.showToast("कृपया चेकलिस्ट को पूरा करें");
      //   } else {
      //     this.httpClient.showToast("Please complete the checklist");
      //   }
      //   this.btnAddSprinkler = false;
    } else if (!this.solarIrrigatedLandArea) {
      if (this.httpClient.currentLanguage == "hindi") {
        this.httpClient.showToast("कृपया पम्प द्वारा सिंचित की जा रही भूमि दर्ज करें");
      } else {
        this.httpClient.showToast("Please enter the Land area being irrigated by pump");
      }
      this.btnAddSprinkler = false;
    } else if (!this.solarSowedCropDetails) {
      if (this.httpClient.currentLanguage == "hindi") {
        this.httpClient.showToast("कृपया मौके पर बोई गई फसलों का विवरण दर्ज करें");
      } else {
        this.httpClient.showToast("Please enter the Details of crops sown on the spot");
      }
      this.btnAddSprinkler = false;
    } else if (!this.solarAnyOtherPoints) {
      if (this.httpClient.currentLanguage == "hindi") {
        this.httpClient.showToast("कृपया अन्य कोई महत्वपूर्ण बिन्दु जो मौका निरिक्षण के दौरान देखा गया है, दर्ज करें");
      } else {
        this.httpClient.showToast("Please enter, Any other important points which noticed during the spot inspection");
      }
      this.btnAddSprinkler = false;
    } else if (
      test.length > 0 ||
      ...
      if (this.httpClient.isOffline) {
        ...
        var sendRequestData = {
          obj: {
            ...
            psw: "rajkisan@123",
            srvnm: "PostVerification",
            srvmethodnm: "SolarPostVerification",
            srvparam: JSON.stringify({
              ImgFarmerWithOfficer: '',
              NotAcceptanceDocPath: '',
              ImgFarmerWithOfficerDateTime: '',
              NotAcceptanceDocPathDateTime: '',
              HortiSLRPVIrrigationSystem: '',
              HortiSLRHorticultureActivities: '',
              IsFarmerAcceptance: this.IsFarmerAcceptance,
              ApplicationId: this.data.ApplicationId,
              latitude: this.latitude,
              longitude: this.longitude,
              ...
              IsOnline_Offline: "1",
              mobilToVerify: "1",
              HortiSLRirrigatedland: self.solarIrrigatedLandArea,
              HortiSLRCropsSownOnSpot: self.solarSowedCropDetails,
              HortiSLROtherImportant: self.solarAnyOtherPoints,
              KhasraNo: this.selectedKhasaraNumber,
              Remarks: '',
              ChecklistDetailList: self.ChecklistDetailList,
            })
          ...
          this.dbService.storage
            ...
            .then((res) => {
              ...
              if (param == 0) {
                ...
                temp.ImgFarmerWithOfficer3 = imageData;
                temp.optionOfImgFarmerWithOfficer3 = options;
              } else if (param == 3) {
                temp.ImgSolar10_2 = imageData;
                temp.optionOfImgSolar10_2 = options;
              } else if (param == 4) {
                temp.ImgSolar10_2 = imageData;
                temp.optionOfImgSolar10_2 = options;
              }
              ...
              this.dbService.storage
                ...
                .then(() => {
                  if (param == 0) {
                    ...
                    this.ImgFarmerWithOfficer3 = imageData;
                  } else if (param == 3) {
                    this.ImgSolar10_2 = imageData;
                  } else if (param == 4) {
                    this.ImgSolar10_2 = imageData;
                  }
                })
              ...
              (data) => {
                ...
                if (temp[0].data[0].URL) {
                  if (param == 0) {
                    ...
                    this.ImgFarmerWithOfficer3 = temp[0].data;
                  } else if (param == 3) {
                    this.ImgSolar10_2 = temp[0].data;
                  } else if (param == 4) {
                    this.ImgSolar10_2 = temp[0].data;
                  }
                } else {
  ...
  checkAvailSubsidy(i) {
    ...
    if (temp.length > 0) {
      if (temp[0].IsChecked == true || temp[0].IsChecked == 'True') {
  ...
  submitFinalData() {
    ...
    if (this.ImgFarmerWithOfficer == null) {
      if (this.httpClient.currentLanguage == "hindi") {
        this.httpClient.showToast(
          "किसान के साथ सिस्टम की तस्वीर।"
        );
      } else {
        this.httpClient.showToast(
          "Image of System with Farmer."
        );
      }
      this.btnFinalSubmit = false;
      ...
      if (this.httpClient.currentLanguage == "english") {
        this.httpClient.showToast(
          "Image with Irrigation System."
        );
      } else {
        this.httpClient.showToast(
          "सिंचाई प्रणाली के साथ तस्वीर।"
        );
      }
      this.btnFinalSubmit = false;
    } else if (this.ImgFarmerWithOfficer3 == null) {
      if (this.httpClient.currentLanguage == "english") {
        this.httpClient.showToast(
          "Image of Solar Controller."
        );
      } else {
        this.httpClient.showToast(
          "सौर नियंत्रक की तस्वीर।"
        );
      }
      this.btnFinalSubmit = false;
    } else if (this.ImgSolar10_2 == null) {
      if (this.httpClient.currentLanguage == "english") {
        this.httpClient.showToast(
          "Capture Form 10.2"
        );
