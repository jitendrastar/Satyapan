...
declare var $: any;
@Component({
  selector: 'app-shade-net-house-post-verification',
  templateUrl: './shade-net-house-post-verification.page.html',
  styleUrls: ['./shade-net-house-post-verification.page.scss'],
})
export class ShadeNetHousePostVerificationPage implements OnInit {

  data:any;
  basicDataSubmitFlag = true;
  isslected_land_for_dealer:any;
  checks_always:0;
...
takePhotoDirect() {
  this.camera.getPicture(this.httpClient.options).then(
      (imageData) => {
          ...
          if (this.httpClient.isOffline) {
              this.dbService.storage.executeSql('SELECT * FROM postVerificationOfflineOfShadeNetHouse WHERE ApplicationId = ?',
                  ...
                  this.dbService.storage.executeSql(`UPDATE postVerificationOfflineOfShadeNetHouse SET applicationData = ? WHERE ApplicationId = ${this.data.ApplicationId}`, data)
                      .then(() => {
...
fill_input(event){
    //alert//(event.getAttribute('class'));
       // alert(event);
     // alert($(".for_select"+event).attr('lang'));
     // console.log(valss);
     this.isslected_land_for_dealer =  $(".for_select"+event).attr('lang');
     this.checks_always = $(".for_select"+event).attr('lang');
    }
    val_check(val_great){
     //alert(val_great);
        if( val_great <= this.checks_always){
            //this.isslected_land_for_dealer="scs";
        }else{
            this.isslected_land_for_dealer="";
        }
  ...
  console.log(checkArray);
   ...
   if (this.longitude == null ||this.longitude == null ||this.longitude  == null) {
    ...
    this.btnAddSprinkler = false;
      ...
      this.btnAddSprinkler = false;
  } else if (this.selectedLandArea == null || this.selectedLandArea == '' || typeof this.selectedLandArea == "undefined") {
      if (this.httpClient.currentLanguage == 'hindi') {
          this.httpClient.showToast('कृपया भूमि क्षेत्र का चयन करें');
      } else {
          this.httpClient.showToast('Please select the Land Area');
      }
      this.btnAddSprinkler = false;
                      ...
                      'obj': {
                          ...
                          srvparam: JSON.stringify({
                             ...
                             // dealerid: 0,
                              ...
                              HectareModel: self.selectedLandArea,
                              optionalArray: [],
...
takePhotoNoCase(param, columnName) {
  ...
  this.camera.getPicture(this.httpClient.options).then(
      (imageData) => {
          ...
          if (this.httpClient.isOffline) {
              this.dbService.storage.executeSql("SELECT * FROM postVerificationOfflineOfShadeNetHouse WHERE ApplicationId = ?", [this.data.ApplicationId]).then((res) => {
                  console.log("second");
                  var temp = JSON.parse(res.rows.item(0).applicationData);
                  console.log("third");
                  if (param == 1) {
                  ...
                  let data = temp;
                  this.dbService.storage.executeSql(`UPDATE postVerificationOfflineOfShadeNetHouse
...
submitNoForm() {
  ...
  if (this.latitude == null ||this.latitude == null ||this.latitude  == null) {
    this.getLoc();
      if (this.httpClient.currentLanguage == "hindi") {
          this.httpClient.showToast(
              "कृपया इस एप्लिकेशन के लिए लोकेशन सक्षम करें फिर सबमिट करें"
            ...
            if (this.httpClient.isOffline) {
                this.dbService.storage.executeSql('SELECT * FROM postVerificationOfflineOfShadeNetHouse WHERE ApplicationId = ?',
                    ...
                    if (param == 0) {
                      ...
                      temp.optionOfImgSubsidyInstalledPlaceFarmerWithOfficer1 = options;
                  } else if (param == 3) {
                      temp.ImgSubsidyInstalledPlaceFarmerWithOfficer2 = imageData;
                      temp.optionOfImgSubsidyInstalledPlaceFarmerWithOfficer2 = options;
                  } else if (param == 4) {
                      temp.ImgSubsidyComponentFarmerWithOfficer1 = imageData;
                      temp.optionOfImgSubsidyComponentFarmerWithOfficer1 = options;
                  }
                  else if (param == 5) {
                      temp.ImgSubsidyComponentFarmerWithOfficer2 = imageData;
                      temp.optionOfImgSubsidyComponentFarmerWithOfficer2 = options;
                  }
                  else if (param == 6) {
                      temp.ImgInsuranceCertificateCopy = imageData;
                      temp.optionOfImgInsuranceCertificateCopy = options;
                  }
                  else if (param == 7) {
                      temp.ImgFarmerSatisfactionConsentLetter = imageData;
                      temp.optionOfImgFarmerSatisfactionConsentLetter = options;
                  }else if (param == 8) {
                      temp.ImgPVChecklistDocument = imageData;
                      temp.optionOfImgPVChecklistDocument = options;
                  }
                    let data = [JSON.stringify(temp)];
                    this.dbService.storage.executeSql(`UPDATE postVerificationOfflineOfShadeNetHouse SET applicationData = ? WHERE ApplicationId = ${this.data.ApplicationId}`, data)
                        .then(() => {
                            if (param == 0) {
                              ...
                              this.ImgSubsidyInstalledPlaceFarmerWithOfficer1 = imageData;
                          } else if (param == 3) {
                              this.ImgSubsidyInstalledPlaceFarmerWithOfficer2 = imageData;
                          } else if (param == 4) {
                              this.ImgSubsidyComponentFarmerWithOfficer1 = imageData;
                          }
                          else if (param == 5) {
                              this.ImgSubsidyComponentFarmerWithOfficer2 = imageData;
                          }
                          else if (param == 6) {
                              this.ImgInsuranceCertificateCopy = imageData;
                          }
                          else if (param == 7) {
                            this.ImgFarmerSatisfactionConsentLetter = imageData;
                        }else if (param == 8) {
                          this.ImgPVChecklistDocument = imageData;
                      }
                        })
                        .catch((e) => {
                            console.log('error ' + JSON.stringify(e));
