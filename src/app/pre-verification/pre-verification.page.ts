...
export class PreVerificationPage {
    ...
    this.httpClient.post(sendRequestData).subscribe(
      function (res: any) {
        ...
        if (res[0].status == 0) {
          ...
          //storage.set('subsidyData',res[0].data);
          self.storage.set("subsidyData", self.getSubsidyDashboardData);
  ...
  onSubsidyClick(subsidyId, count) {
    ...
    if (subsidyId == 12) {
      this.router.navigate(['barbed-wire-fencing', { subsidyId: subsidyId }])
    }else if (subsidyId == 62) {
      this.router.navigate(['whs', { subsidyId: subsidyId }])
    }
    else if (this.header == "1") {
  ...
  assignedToAs() {
    this.router.navigate(["aao-verification", { header: 1 }]);
  }
  assignedToAAO() {
    //alert();
    this.router.navigate(["aao-verification", { header: 2 }]);
  }
